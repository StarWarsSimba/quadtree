...
def build_quad_tree(grid: list[list[str]], row_l: int, col_l: int, size: int) -> QuadTree:
    ...
    if size == 1:
        if grid[row_l][col_l] == BLACK_PIXEL:
            return BLACK_NODE
        else:
            return WHITE_NODE
    else:
        if all_match(grid, row_l, col_l, size, BLACK_PIXEL):
            ...
            half_size = size // 2
            return GreyNode(build_quad_tree(grid, row_l, col_l, half_size),
                            build_quad_tree(grid, row_l, col_l + half_size, half_size),
                            build_quad_tree(grid, row_l + half_size, col_l + half_size, half_size),
